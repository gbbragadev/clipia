...
@pytest.mark.asyncio
async def test_generate_limit_is_per_ip_and_does_not_block_other_ips(
    client_factory, db_session, verified_user, auth_headers
):
    ...
    assert responses_a[9].status_code == 202, "The tenth generate request should still be accepted."
    assert responses_a[10].status_code == 429, "The eleventh generate request from one IP should be rate limited."
    assert response_b.status_code == 202, "A different IP should not be blocked by another IP's rate limit."


@pytest.mark.asyncio
async def test_resend_code_limit_returns_429(client, unverified_user):
    responses = [await client.post("/api/v1/auth/resend-code", json={"email": unverified_user.email}) for _ in range(4)]
