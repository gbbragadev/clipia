...
| P2 | Problema | Cookie/CSRF em duas releases e hardening de CORS/hosts/metrics/docs/CSP continuam pendentes. | Auth e infraestrutura | Auditoria técnica I; código atual. | Alta | Crítico | XL | Telemetria de compatibilidade verde e produção simulada bloqueando superfícies indevidas. |
...
| SEC-03 | P2 | Problema | Hardening de CORS/hosts/metrics/docs/CSP por ambiente. | Infra | Seção I. | Alta | Alto | M | Produção simulada bloqueia superfície e cardinalidade indevidas. |
...
| 90 dias | Problema | Após concluir localmente assets/upload/reset/consentimento, restam infra hardening, ledger shadow, cookie+CSRF e deploy versionado. | Segurança/operação | e6225d6, e802fd6, 63169e3 e I/L. | Alta | Crítico | XL | Repetir threat tests no candidato, obter 7 dias de ledger zero e telemetria auth verde. |
