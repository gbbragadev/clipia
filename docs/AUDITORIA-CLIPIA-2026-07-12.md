...
| Fato confirmado | A base first-party está implementada e desabilitada por padrão: tabela append-only, endpoint estrito, 15 eventos cliente e 10 eventos servidor tipados, emissão transacional idempotente, sessão aleatória em sessionStorage, RBAC admin, funil de nove etapas e coortes por semana/origem/nicho/dispositivo. | Analytics/API/admin | Commits acb647c, a41d240, c8e6388 e 0544007; migrations f3a4b5c6d7e8/c6d7e8f9a0b1, 50 testes focados finais, PostgreSQL concorrente, typegen/tsc e build isolado. | Alta local | Alto | M | Manter as duas flags off até aprovação de Privacidade; publicar no candidato e coletar 14 dias antes do onboarding novo. |
...
|---|---|---|---|---|---|---|---|
| Fato confirmado | Funil visita → CTA → cadastro → verificação → primeira geração → exportação → checkout → pagamento → segunda geração solicitada está implementado. | Admin | 0544007; dashboard, sessão vinculada e testes de coorte. | Alta local | Alto | S | Repetir no candidato; visitas/CTA só viram baseline com flag aprovada. |
...
| Requer analytics | Receita já reconcilia paid/refunded/void sem pending como receita; COGS/fatura, ARPPU e margem ainda dependem de dados externos. | Admin financeiro | e64d217, 0544007 e ausência de faturas reconciliadas. | Alta | Crítico | L | Fatura e custo por operation_id antes de claim de margem. |
