...
| Requer analytics | Não é possível afirmar se o maior vazamento está em CTA, cadastro, verificação, primeira geração, exportação ou pagamento. | Funil de ativação e receita | Taxonomia first-party e agregados reconciliados ainda inexistentes. | Alta | Alto | L | Coletar 14 dias do funil autoritativo por origem, nicho e dispositivo. |
...
| Fato confirmado | A Release B corrigiu localmente intenção de pacote perdida, largura móvel forçada, nichos sem exemplo e crédito abstrato; a ausência de funil first-party confiável permanece. | Cadastro, mobile, nichos, preços e analytics | Commits 162d8cf, 9d6eed6, aa71128 e 095fe83; 26 Playwright verdes. | Alta local | Alto | L | Publicar apenas após gates e concluir analytics atrás de flag. |
...
| P1 | Problema | Não existe analytics first-party autoritativo nem baseline. | Funil inteiro | Ausência de tabela/endpoint/dashboard no snapshot. | Alta | Alto | L | Contrato J, RBAC, coortes e agregados reconciliados atrás de flag. |
| P2 | Problema | Assets por URL/path, reset reutilizável, uploads e auth atual ampliam superfície de ataque. | Mídia, auth e upload | Auditoria técnica I. | Média/alta | Crítico | XL | IDs opacos, reset one-time, streaming validado e migração cookie/CSRF. |
| P2 | Problema | User.credits ainda não tem ledger append-only completo. | Créditos | Modelo atual. | Alta | Crítico | XL | Shadow + backfill + sete dias de diferença zero antes de enforce. |
...
| Problema | musicUrl/paths devem virar IDs opacos server-owned. | Mídia/voz | Contratos atuais. | Média/alta | Crítico | L | Ownership, contenção e rejeição de file:, UNC, localhost, redes privadas e origens fora da allowlist. |
| Problema | Upload deve ser streaming, com limite antecipado, MIME e magic bytes. | Upload | Implementação atual. | Alta | Alto | M | Arquivo inválido/grande encerra antes do buffer completo. |
| Problema | Reset exige JTI/used_at; consentimento deve ser obrigatório/versionado. | Auth/legal | Modelos/rotas atuais. | Média/alta | Alto | L | Replay falha e cadastro sem versão de consentimento falha. |
| Problema | CORS/hosts, metrics, docs, CSP e labels precisam de hardening por ambiente. | Infra | Config/observabilidade atuais. | Média/alta | Alto | M | Produção simulada bloqueia superfície indevida e cardinalidade. |
| Problema | Redis nunca pode publicar sucesso se persistência final no DB falhar. | Worker/status | Invariante operacional. | Alta | Crítico | M | Falha DB forçada deixa estado não-terminal e alerta. |
| Problema | Auth migra para cookie HttpOnly/Secure/SameSite=Lax + CSRF em duas releases. | Sessão | Token atual. | Alta | Alto | L | Bearer compatível primeiro; localStorage removido só após telemetria verde. |
...
| Problema | Toda mutação de crédito deve ter idempotency key e delta SQL na transação que registra origem. | Créditos | 1A/1B; ledger futuro. | Alta | Crítico | L | Nenhum delta sem origem; reconciliável diariamente. |
...
| Problema | User.credits continua projeção autoritativa durante ledger shadow; Redis é cache/telemetria, nunca autoridade financeira. | Ledger/status | Plano de migração. | Alta | Crítico | XL | Sete dias zero antes de enforce; rollback mantém projeção. |
...
|---|---|---|---|---|---|---|---|
| Hipótese | Tabela PostgreSQL append-only com event_id UUID, schema_version, occurred_at/received_at, anonymous_session_id, user opcional server-derived, page, attribution, device_class e properties validadas. | Analytics | Plano aprovado; implementação pendente. | Alta | Alto | L | INSERT only no fluxo normal; nomes/props extras 422; event_id único. |
| Problema | POST /api/v1/analytics/events aceita auth opcional, até 20 eventos/64 KB, rate limit e rejeição estrita. | API | Contrato aprovado, rota inexistente. | Alta | Alto | M | Lote excessivo 413; replay idempotente; sem campos desconhecidos. |
| Problema | O schema futuro deve permitir somente utm_source, utm_medium, utm_campaign, utm_content e utm_term, strings normalizadas de até 100 caracteres; source deve ser derivado no servidor como direct/organic/search/social/referral/email/paid/unknown. | Aquisição | Decisão de schema ainda não implementada. | Alta | Alto | S | Rejeitar a sexta chave/valor fora do limite e não aceitar source arbitrário do cliente. |
| Problema | O payload futuro não aceitará PII direta (e-mail, nome, telefone, IP, UA bruto, cartão, token, texto livre), mas session_id e user_id server-derived são dados pseudonimizados, não dados anônimos. | Privacidade | Requisitos/LGPD ainda não implementados. | Alta | Crítico | M | Inventário, retenção, acesso e exclusão tratam pseudônimos como dados pessoais. |
...
| Problema | Agregados de jobs, créditos, refunds e compras precisam ser corrigidos antes do baseline. | Dashboard | Uma linha de job/purchase não equivale a geração paga/receita. | Alta | Crítico | L | Totais reconciliam com estados autoritativos e não contam pending como receita. |
...
|---|---|---|---|---|---|---|---|---|---|
| Envelope comum | Problema | Cliente envia somente event_id, schema_version, occurred_at, anonymous_session_id, page, device_class e UTMs permitidas; servidor deriva received_at, user_id e dados de rede não persistidos. | event_id:UUID; schema_version:int=1; occurred_at:datetime; anonymous_session_id:UUID; page:enum de rota; device_class:desktop/mobile/tablet/unknown; utm_source/utm_medium/utm_campaign/utm_content/utm_term somente em aquisição. | Todos os eventos | Contrato aprovado; endpoint ainda não implementado. | Alta | Crítico | M | Campo comum extra é 422; user_id/IP/UA bruto do cliente são rejeitados; cada linha abaixo restringe ainda mais as properties. |
...
| ANA-01 | P1 | Problema | Tabela/endpoint first-party atrás de flag. | Analytics | Seção J. | Alta | Alto | L | Allowlist por evento, 20/64 KB, rate limit. |
...
| SEC-01 | P2 | Problema | IDs opacos, SSRF/path, upload streaming. | Mídia/upload | Seção I. | Média/alta | Crítico | L | Payloads maliciosos rejeitados. |
| SEC-02 | P2 | Problema | Reset one-time, consentimento versionado, infra hardening. | Auth/infra | Seção I. | Alta | Alto | L | Threat/regression tests. |
| LED-01 | P2 | Problema | Ledger append-only em shadow. | Créditos | Plano. | Alta | Crítico | XL | Delta/origem/referências/idempotency/balance_after. |
| LED-02 | P2 | Problema | Backfill/reconciliação; enforce após 7 dias zero. | Créditos | Plano. | Alta | Crítico | L | Rollback mantém User.credits. |
| AUTH-01 | P2 | Problema | Cookie HttpOnly + CSRF em duas releases. | Sessão | Plano. | Alta | Alto | L | Remover Bearer/localStorage após telemetria. |
...
| 30 dias | Requer analytics | Publicar analytics após Privacidade, corrigir agregados, coletar 14 dias e só então onboarding/experimento. | Funil | H/J/K. | Alta | Alto | XL | Sem PII direta, RBAC e coortes. |
...
| 90 dias | Problema | Assets/upload/reset/consentimento/infra; ledger shadow; cookie+CSRF; deploy versionado. | Segurança/operação | I/L. | Alta | Crítico | XL | Threat tests, 7 dias ledger zero e telemetria auth verde. |
...
| Problema | PII direta/evento duplicado e acesso indevido. | Analytics/admin | Auditoria de payload, unique event_id e RBAC. | Zero contínuo. | Alta | Crítico | L | Campos proibidos 422; replay zero-delta; não-admin 403. |
